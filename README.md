...
> 把仓库连接到 Cloudflare Pages，每次推送代码自动完成构建和部署，不需要在本地执行任何命令。

---

### 第一步：获取 Cloudflare 账号信息

部署前需要先准备两个信息：**账号 ID** 和 **API 令牌**。

**账号 ID 在哪里找：**
1. 打开 [Cloudflare 控制台](https://dash.cloudflare.com)，登录后进入首页
2. 右侧边栏向下滚动，可以看到「账户 ID」，点击复制

**API 令牌怎么创建：**
1. 点击右上角头像 → **我的个人资料**
2. 左侧点击 **API 令牌** → 点击右上角 **创建令牌**
3. 找到「编辑 Cloudflare Workers」模板，点击右侧 **使用模板**
4. 直接点击页面底部 **创建令牌**
5. 复制页面上显示的令牌字符串（**只显示一次，务必保存好**）

---

### 第二步：创建 Pages 项目并连接仓库

1. 在 Cloudflare 控制台左侧点击 **Workers 和 Pages**
2. 点击右上角 **创建** → 选择 **Pages** 标签
3. 点击 **连接到 Git**，按提示授权 GitHub 账号
4. 选择你 Fork 的 `cloud-mail` 仓库，点击 **开始设置**

---

### 第三步：填写构建配置

在「设置构建和部署」页面，找到以下字段并填写：
...
在同一页面继续向下，找到「环境变量」区域，点击 **添加变量**，把下面的变量逐条填进去：

**必填（缺少任何一项都会部署失败）：**

| 变量名 | 填什么 | 示例 |
|--------|--------|------|
| `CLOUDFLARE_API_TOKEN` | 第一步复制的 API 令牌 | `abc123def456...` |
| `CLOUDFLARE_ACCOUNT_ID` | 第一步复制的账号 ID | `1a2b3c4d...` |
| `JWT_SECRET` | 你自己设置的登录密钥，20位以上随机字母数字，**不能包含 `? % # / \`** | `MySecret2024xyz` |
| `DOMAIN` | 你用来收邮件的域名，**格式必须带方括号和引号** | `["example.com"]` |
| `ADMIN` | 管理员邮箱，部署后用这个邮箱登录后台 | `admin@example.com` |

**选填（不填则关闭对应功能）：**

| 变量名 | 作用 |
|--------|------|
| `R2_BUCKET_NAME` | 附件存储桶名称，不填则无法收发附件 |
| `CUSTOM_DOMAIN` | 绑定自定义域名，如 `mail.example.com` |
| `D1_DATABASE_ID` | D1 数据库 ID，不填则自动创建 |
| `KV_NAMESPACE_ID` | KV 命名空间 ID，不填则自动创建 |
...
所有内容填完后，点击页面底部 **保存并部署**，等待 2～3 分钟。

部署日志最后出现 `Deployed cloud-mail` 即表示成功。
...
```
https://你的访问域名/api/init/你的JWT_SECRET值
```

**举例：** 假设访问域名是 `cloud-mail.abc.workers.dev`，JWT_SECRET 设的是 `MySecret2024xyz`，则访问：

```
https://cloud-mail.abc.workers.dev/api/init/MySecret2024xyz
```

页面显示 `success` 表示初始化完成。之后用 `ADMIN` 里填的邮箱登录即可。

> 以后每次更新代码只需推送到仓库，Cloudflare Pages 会自动重新部署，无需重复以上步骤。
